...
const entries = [
  { src: "lebon-chablis-source.png", id: "lebon-chablis" },
  { src: "lebon-irancy-source.png", id: "lebon-irancy" },
  { src: "lebon-brulis-source.png", id: "lebon-brulis" },
];

(async () => {
  for (const { src, id } of entries) {
    const buf = fs.readFileSync(path.join(__dirname, "..", "public", "photos", src));
    ...
    const outBuf = await resized
      ...
      .toBuffer();
    fs.writeFileSync(path.join(outDir, `${id}.png`), outBuf);
